...
pub fn generate_ast(env: &mut PengEnv, ast: &PengAST) -> Result<PengUnit, PengError> {
...
pub fn generate_ast_using(
    ...
    match ast {
        PengAST::Script(statements) => generate_script_using(env, statements, using_unit),

        PengAST::Program(declarations) => generate_program_using(env, declarations, using_unit),
    }
}
